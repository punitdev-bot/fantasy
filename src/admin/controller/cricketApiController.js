...
class cricketApiController {
  ...
  listOfMatches(req, res) {
    try {
      let url = `https://rest.entitysport.com/v2/matches/?status=1&token=5a59a52147983277808ffc8ee228c2bd&per_page=70&&paged=1`;
    ...
    for (let obkey1 of Object.values(result.data)) {
      obkey1.matches.flatMap(async (Obj) => {
        ...
        if (checkMatchkey.length == 0) {
          let url = `http://rest.entitysport.com/v2/matches/${Obj.matchkey}/info?token=5a59a52147983277808ffc8ee228c2bd`;
    ...
    for (let mymatch of items) {
      ...
      if (checkMatchkey.length == 0) {
        ...
        if (
          ...
          let temaData1, temaData2, series;
          let getTeam1 = await teamModel.findOne({ team_key: mymatch.teama.team_id });
...
logoImage = mymatch.teama.logo_url
       ...
       logoImage = mymatch.teama.logo_url
          ...
          let getTeam2 = await teamModel.findOne({ team_key: mymatch.teamb.team_id })
