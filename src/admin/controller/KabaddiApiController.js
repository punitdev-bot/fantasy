...
class KabaddiApiController {
  ...
  async listOfMatches_entity(req, res) {
    try {
      ...
      let url = `https://kabaddi.entitysport.com/matches?token=068cdc7a4981de1b3f78309fd6b306d0`;
        ...
        //   )
          ...
          if (await teamModel.findOne({ team_key: mymatch.teams.home.tid })) {
            ...
            temaData1 = await teamModel.findOneAndUpdate(
              ...
              {
                $set: {
                  teamName: mymatch.teams.home.tname,
                  short_name: mymatch.teams.home.shortname,
                },
              },
              { new: true }
            );
          ...
          if (await teamModel.findOne({ team_key: mymatch.teams.away.tid })) {
            temaData2 = await teamModel.findOneAndUpdate(
              ...
              {
                $set: {
                  teamName: mymatch.teams.away.tname,
                  short_name: mymatch.teams.away.shortname,
                },
              },
              { new: true }
            );
...
logoImage = mymatch.teams.home.logo
       ...
       logoImage2 = mymatch.teams.away.logo
          ...
          const updateListMatch = await listMatchModel.findOneAndUpdate(
            { real_matchkey: mymatch.mid, "fantasy_type": fantasy_type },
            {
              $set: {
                name: mymatch.title,
                short_name: mymatch.shorttitle,
  ...
  fetchPlayerByMatch_entity(req, res) {
    try {
      ...
      axios
        .get(
          `http://rest.entitysport.com/v2/matches/${req.params.matchkey}/squads?token=1&token=5a59a52147983277808ffc8ee228c2bd`
        )
...
module.exports = new KabaddiApiController();
